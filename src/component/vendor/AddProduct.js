...
const AddProduct = () => {
  ...
  const [category, setCategory] = useState('');
  const [image, setImage] = useState(null);
  const [productCompany, setProductCompany] = useState(''); // New state for productCompany
  const [error, setError] = useState(null);
  
  // Retrieve vendorId from local storage
  const vendorId = localStorage.getItem('customerId'); // Assuming you need vendorId, not customerId
  ...
  const handleFormSubmit = (event) => {
    ...
    const formData = new FormData();
    formData.append('vendorId', vendorId); // Add vendorId to formData
    formData.append('productName', productName);
    formData.append('productDescription', productDescription);
    formData.append('price', price);
    formData.append('category', category);
    formData.append('productCompany', productCompany); // Add productCompany to formData
    formData.append('image', image);
    ...
    fetch('http://localhost:8080/api/product/add/product', {
      ...
      .then((data) => {
        ...
        setPrice(0.0);
        setCategory('');
        setProductCompany(''); // Reset productCompany
        setImage(null);
          ...
          <div className="form-row" style={{ marginLeft: "10px" }}>
            <div className="form-group">
              ...
              <input
                ...
                onChange={(event) => setProductCompany(event.target.value)}
              />
            </div>
          </div>
          <div className="form-group" style={{ marginLeft: "450px", width: "100px", marginTop: "40px" }}>
